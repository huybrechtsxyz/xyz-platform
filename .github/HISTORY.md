...
- **Root cause**: `TerraformBuilder._save_terraform_vars()` gated `flags.auto.tfvars.json`/`variables.auto.tfvars.json`/`properties.auto.tfvars.json`/`custom.auto.tfvars.json` emission on `profile is not None and profile.should_emit(...)`. When a provisioner declares no `output:` block at all (`prov.output` is `None` — a common, minimal, and until now seemingly-valid configuration), every one of those checks short-circuited to `False`, so none of those files were ever written — even though `OutputProfileModel`'s own docstring states the default behaviour (absent profile) should be equivalent to `format: strata` (emit everything). The failure was silent: `build run` succeeded with no errors or warnings, and the gap only surfaced downstream as "required variable not supplied" failures inside the actual `terraform plan`/`apply` step.
- **Fix**: changed each condition from `profile is not None and profile.should_emit(...)` to `profile is None or profile.should_emit(...)` — an absent profile now emits unconditionally (matching the documented default); an explicit profile still fully controls emission via `should_emit()`.
- **Testing**: new `TestSaveTerraformVarsNoProfile.test_absent_output_profile_emits_properties_variables_features` in `test_builders_output_profile.py`, asserting `properties`/`variables`/`flags` tfvars files are written and populated when `prov.output` is `None`.

#### **`AnsibleBuilder` generated a spurious `ansible/` build folder for workspaces with zero `ansible` provisioners**

- **Root cause**: `AnsibleBuilder.build()` unconditionally called `_build_ansible_vars()`/`_save_ansible_vars()` regardless of whether the workspace declared any `ProvisionerType.ANSIBLE` provisioner at all. `_save_ansible_vars()`'s `_resolve_ansible_paths()` correctly returns an empty list when there are zero Ansible provisioners, but the caller's fallback (`if not ansible_paths: ansible_paths = [deployment_service.get_build_path(build_path) / "ansible"]`) then wrote `strata_workspace.yml`/`strata_providers.yml`/`strata_topologies.yml` into that default path anyway — conflating "no Ansible provisioner declared" with "Ansible provisioner(s) exist but `solution_controller` couldn't resolve their canonical path" (the fallback's actual intended use case). Confirmed via a real Terraform-only workspace (`cfg-int-deployment`'s `control_workspace`, one `terraform` provisioner, zero `ansible` provisioners) whose build output nonetheless contained a populated `ansible/` directory.
- **Fix**: new `AnsibleBuilder._has_ansible_provisioner()` helper, checked at the top of `build()` (both dry-run and real-run paths) — when the workspace has zero `ANSIBLE` provisioners, `build()` returns `True` immediately without calling `_build_ansible_vars()`/`_save_ansible_vars()` at all, so no `ansible/` folder is created. The `_save_ansible_vars()` fallback itself is unchanged (still correct for its original intended case: Ansible provisioners exist but `solution_controller` is unavailable).
- **Testing**: new `test_no_ansible_provisioner_skips_generation_entirely`/`test_no_ansible_provisioner_skips_generation_in_dry_run` regression tests. Existing `TestAnsibleBuilderBuild` tests' `_mock_svc()` helper updated to explicitly declare an Ansible provisioner (previously relied on an unconfigured `MagicMock` behaving as an empty iterable, which is exactly how this bug went undetected).

#### **`TerraformBuilder` had the identical spurious-output bug as `AnsibleBuilder` above**

- **Audit**: checked every builder (`Bicep`, `Compose`, `Helm`, `Sync`, `Terraform`) for the same "generate/write output regardless of whether a matching provisioner or module actually exists" pattern. `BicepBuilder`/`ComposeBuilder`/`HelmBuilder`/`SyncBuilder` are all clean — each correctly no-ops (returns `True`, writes nothing) when there's no matching provisioner/module/stage; they never have a "no results resolved → fall back to a default path" branch. `TerraformBuilder._save_terraform_vars()` had the exact same bug as `AnsibleBuilder`: `if not provisioners: # Fallback for workspaces with no provisioner list loaded` wrote a default `terraform/` folder unconditionally, conflating "the provisioner list is genuinely empty" with its intended narrower case ("couldn't determine the list"). No live repro found (nearly every real workspace declares ≥1 Terraform provisioner), but the identical fix was applied for consistency and to close the gap before a Bicep/Compose/Helm-only workspace ever hits it.
- **Fix**: new `TerraformBuilder._has_terraform_provisioner()` helper (mirrors `AnsibleBuilder`'s), checked at the top of `build()` — when the workspace has zero `TERRAFORM` provisioners, `build()` returns `True` immediately without calling `_build_terraform_vars()`/`_save_terraform_vars()`. `_save_terraform_vars()`'s own fallback branch is unchanged (now effectively dead code reachable only via direct unit-test calls, not through `build()`).
- **Testing**: new `test_no_terraform_provisioner_skips_generation_entirely`/`test_no_terraform_provisioner_skips_generation_in_dry_run` regression tests; `_mock_svc()` in `test_builders_terraform.py` updated the same way as the Ansible fixture.
